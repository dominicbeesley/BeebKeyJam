...
const uint CAPTURE_N_SAMPLES = 800;
...
int main() {
    ...
    logic_analyser_init(pio, sm, CAPTURE_PIN_BASE, CAPTURE_PIN_COUNT, (float)clock_get_hz(clk_sys) / (16000000));
    ...
    while(1) {
        ...
        logic_analyser_arm(pio, sm, dma_chan, capture_buf, buf_size_words, CAPTURE_PIN_SYNC, false);
